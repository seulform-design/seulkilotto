...
function ManualNumberGrid({
  ...
  const reviewMode = Boolean(winningSet?.size);
  return (
    <Box>
      ...
      >
        {GRID_NUMBERS.map((n) => {
          ...
          const isWinner = winningSet?.has(n);
          const dimmed = reviewMode ? !isWinner : !selected;
          return (
            <Box
              key={n}
              onClick={() => onToggle(n)}
              sx={{
                ...
                cursor: 'pointer',
                opacity: reviewMode ? (isWinner ? 1 : 0.45) : selected ? 1 : 0.55,
                transform: selected ? 'scale(1.05)' : 'scale(1)',
                transition: 'transform 0.12s ease, opacity 0.12s ease',
                outline: selected ? '2px solid #FBC400' : undefined,
                outlineOffset: 2,
                borderRadius: '50%',
              }}
            >
              <LottoBall number={n} size={36} dimmed={dimmed} />
            </Box>
          );
        })}
      </Box>
    ...
    <Stack spacing={2}>
      ...
      <Paper sx={{ p: 2 }}>
        ...
        <ManualNumberGrid
          ...
          winningSet={activeReviewWinningSet}
        />
