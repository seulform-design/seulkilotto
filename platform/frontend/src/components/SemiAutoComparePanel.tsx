...
interface SemiAutoComparePanelProps {
  ...
  /** @deprecated 데이터 층은 verificationSlot(④ 검증)으로 이전 */
  analysisEpilogue?: ReactNode;
  /** ④ 패턴 엔진 · 검증 탭 — 용지 정화 · 의도별 누적 · 복기검증·백테스트 */
...
export default function SemiAutoComparePanel({
  ...
  const reviewVerificationQuery = useQuery({
    ...
    queryKey: ['v1-photo-review-verification', 'expand24-v14-prefer-expand', 'pair-product'],
    queryFn: v1Api.getReviewVerification,
    staleTime: 60_000,
    retry: 1,
  });
  const graftCoverageQuery = useQuery({
    queryKey: ['v1-photo-graft-coverage', 'graft-v13-prefer-expand', sheetIntent],
    queryFn: () =>
  ...
  const graftCoverageEV = useMemo(() => {
    ...
    if (api?.ok && (api.core6?.length ?? 0) >= 6 && (api.expand24?.length ?? 0) >= 6) {
      ...
      return {
        ...
        graftBuild: api.graft_build ?? 'graft-v13-prefer-expand',
  ...
  return (
    <Stack spacing={2}>
      ...
      {/* 레거시 슬롯 — 데이터 층은 verificationSlot(④ 검증)으로 이전됨 */}
      {analysisEpilogue}
        </>
        )}
      </Paper>
      ...
      {(decadePattern || finalStrongExpected) && (
        <>
              {/* ★ 강수 & 기대수 (구간별 · 신호 종합) — 구 '1:1 강수&기대'(반복도) + '최종 강수·기대'
                  (검증학습·이월 재정렬)를 하나로 통합. 반복도 줄수·끝수·당첨분포 + 배지·합의를 한 섹션에. */}
              {finalStrongExpected && (
                <Box sx={{ mt: 1.25, p: 1, borderRadius: 1, border: '1px solid', borderColor: 'warning.main' }}>
                  ...
                  <Stack spacing={0.5}>
                    {finalStrongExpected.bands.map((b) => (
                      <Stack key={`fse-${b.label}`} direction="row" alignItems="center" spacing={0.75} flexWrap="wrap" useFlexGap>
                        ...
                        {b.expected.map((s) => (
                          <Box key={`fse2-${s.number}`} sx={{ textAlign: 'center', minWidth: 34 }}>
                            <LottoBall number={s.number} size={ENGINE_BALL.list} dimmed={compareWinning && winningSet ? !s.winning : false} />
                            <Typography variant="caption" sx={{ display: 'block', fontSize: 8, lineHeight: 1, color: 'text.disabled' }}>
                              {s.auto}/{s.semi}
                            </Typography>
                  ...
                  {/* 끝수 · 당첨 분포 — 구 '★ 1:1 강수 & 기대수' 에서 통합(중복 섹션 합침) */}
                  {decadePattern && (
                    <Stack direction="row" alignItems="center" spacing={0.5} flexWrap="wrap" useFlexGap sx={{ mt: 0.5 }}>
                      <Typography variant="caption" fontWeight={700} sx={{ minWidth: 44, fontSize: 10 }}>끝수</Typography>
                      {decadePattern.endingTop.map((e) => (
                        <Chip key={`fse-ed-${e.digit}`} size="small" variant="outlined" label={`${e.digit} (${e.count})`} sx={{ height: 18, fontSize: 10 }} />
                      ))}
                    </Stack>
                  )}
                  {compareWinning && decadePattern?.distribution && (
                    <Typography variant="caption" color="text.secondary" sx={{ display: 'block', fontSize: 9, mt: 0.5 }}>
                      <strong>당첨 분포</strong>: 강수 {decadePattern.distribution.strong}개 · 기대 {decadePattern.distribution.expected}개 · 그외 등장 {decadePattern.distribution.etc}개
                      {decadePattern.distribution.missing > 0
                        ? ` · 티켓 밖 ${decadePattern.distribution.missing}개(용지 한계 — 학습 신호 제외)`
                        : ' · 전부 티켓 안(추출 가능 회차)'}
                    </Typography>
                  )}
                  {(() => {
      ...
      <EngineTabBanner
        ...
        chips={
          <>
            ...
            <EngineStatusChip
              ...
              label="의도별 누적"
              onClick={() =>
                document.getElementById('engine-intent-accum')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
              }
              sx={{ cursor: 'pointer' }}
            />
        ...
        intent={
          <>
            <strong>데이터 층</strong>(회차별 용지 정화 → 의도별 누적)을 먼저 확인한 뒤,
