    ...
    <Stack spacing={1.5}>
      <Alert severity="info" icon={false} sx={{ py: 0.75 }}>
        <Typography variant="caption">
          <strong>{intentLabel} {roundNo ?? '?'}회</strong> 기준 — 후속출현·미출현을 엔진에 통합했습니다.
          더 깊은 표·전략 조합은 앱 상단 <strong>후속 출현 통계</strong> 탭에서 동일 엔진으로 이어서 볼 수 있습니다.
          1등 확률(1/8,145,060)은 불변입니다.
        </Typography>
      </Alert>
      ...
      <Paper variant="outlined" sx={{ p: 1.5, borderColor: 'secondary.main' }}>
        <Stack direction="row" alignItems="center" spacing={0.75} flexWrap="wrap" useFlexGap sx={{ mb: 0.75 }}>
          ...
          {postOccurrence?.trigger_round != null && (
            <Chip size="small" variant="outlined" label={`트리거 ${postOccurrence.trigger_round}회`} sx={{ height: 20, fontSize: 10 }} />
          )}
        </Stack>
        ...
        {!postOccurrence?.available ? (
          <Alert severity="warning" sx={{ py: 0.5 }}>후속출현 신호가 아직 없습니다. 재분석 또는 후속 출현 통계 탭에서 분석을 실행하세요.</Alert>
        ) : (
          <Stack spacing={0.75}>
            {(
              [
              ...
              <Stack key={grade} direction="row" alignItems="center" spacing={0.5} flexWrap="wrap" useFlexGap>
                <Chip size="small" color={color} label={`후속 ${grade} ${nums.length}`} sx={{ height: 20, fontWeight: 700, minWidth: 72 }} />
                {nums.length === 0 ? (
      ...
      <Paper variant="outlined" sx={{ p: 1.5, borderColor: 'info.main' }}>
        ...
        <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1 }}>
          {decadeGap?.summary ?? '구간별 미출현(gap)이 긴 번호 풀 — 통합 신호 decade_gap 축과 동일.'}
        </Typography>
        {gapPool.length > 0 ? (
          ...
          <Alert severity="info" sx={{ py: 0.5 }}>구간 미출현 풀이 비어 있습니다.</Alert>
        )}
        {decadeGap?.table && (
          <Stack spacing={0.4} sx={{ mt: 1 }}>
            {Object.entries(decadeGap.table).map(([band, rows]) => (
              <Stack key={band} direction="row" alignItems="center" spacing={0.5} flexWrap="wrap" useFlexGap>
                <Typography variant="caption" fontWeight={700} sx={{ minWidth: 52, fontSize: 10 }}>{band}</Typography>
      ...
      <Paper variant="outlined" sx={{ p: 1.5, borderColor: 'warning.main' }}>
        <Stack direction="row" alignItems="center" spacing={0.75} flexWrap="wrap" useFlexGap sx={{ mb: 0.75 }}>
          ...
          <Chip size="small" variant="outlined" label={`${missingCount}개`} sx={{ height: 20, fontSize: 10, fontWeight: 700 }} />
        ...
        <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1 }}>
          ...
          {compareWinning ? ` ${roundNo ?? '?'}회 당첨이 여기 있으면 그 회차는 용지 분석만으로 못 잡습니다.` : ' 이번회차(미추첨) — 당첨 대조 없음.'}
        </Typography>
        {!missingBands || missingCount === 0 ? (
          <Alert severity="success" sx={{ py: 0.5 }}>용지 미출현이 없습니다(1~45가 티켓에 모두 등장).</Alert>
        ) : (
          <Stack spacing={0.5}>
            {missingBands.map((b) => (
